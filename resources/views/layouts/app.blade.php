...
<header class="w-full container mx-auto">
    <div class="flex flex-col items-center py-12">
        <a class="font-bold text-gray-800 uppercase hover:text-gray-700 text-5xl" href="{{route('home')}}">
            {{ \App\Models\TextWidget::getTitle('under-blog-name') }}
        </a>
        <p class="text-lg text-gray-600">
            {{ \App\Models\TextWidget::getContent('under-blog-name') }}
        </p>
    </div>
</header>
        ...
        <div class="w-full container mx-auto flex flex-col sm:flex-row items-center justify-between text-sm font-bold uppercase mt-0 px-6 py-2">
           <div>
            ...
            <div>
                @auth
                    ...
                    <div class="sm:flex sm:items-center sm:ml-6 text-black">
                        <x-dropdown align="right" width="48">
                            <x-slot name="trigger">
                                <button class="inline-flex items-center px-3 py-2 border border-transparent text-md text-black leading-4 font-bold rounded-md hover:bg-blue-600 hover:text-white focus:outline-none transition ease-in-out duration-150">
                ...
                @else
                    <a href="{{route('login')}}"
                       class="hover:bg-blue-600 hover:text-white rounded py-2 px-4 mx-2 {{ ( request()->route()->getName() == 'login') ? 'bg-blue-600 text-white' : '' }}">
                        Login</a>
                    <a href="{{route('register')}}"
                       class="bg-blue-600 text-white rounded py-2 px-4 mx-2 {{ ( request()->route()->getName() == 'register') ? 'bg-blue-600 text-white' : '' }}">
                        Register</a>
                @endauth
            </div>
        </div>
